...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        ...
        if(AutoRunning){
            ...
            placeBet( parseFloat(parseFloat($('#auto_cashout_input').val()).toFixed(2)), parseFloat($('#auto_bet_input').val()) );
        }
    });
});
...
$('#bet_cashout_button').click(function(){
    ...
    if(!betting){
        placeBet( parseFloat(parseFloat($('#cashout_input').val()).toFixed(2)), parseFloat($('#bet_input').val()) );
    }
});
...
function placeBet(stopAt, bet){
    ...
    $('#bet_cashout_button').attr("disabled", false);

    $('#bust_box').html("<span>"+parseFloat((currentAt<1&&currentAt>0?0:currentAt)).formatMoney(2,'.',',')+"</span>x");
    ...
    getHash(function(){
        ...
        $.ajax({
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                if(lostAt>stopAt) lostAt = parseFloat(parseFloat(stopAt-0.02).toFixed(2));
                var losingBetInterval = setInterval(function(){
                    if(currentAt<lostAt){ // betting
                        ...
                        cell2.innerHTML = bet.formatMoney(2, '.', ',');
