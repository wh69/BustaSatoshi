...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        if(!user.connected){
            window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1280&response_type=token&state=Meh&redirect_uri=https://dexon95.github.io/bustasatoshi/";
            return;
        }
        if(AutoRunning){
            AutoRunning = false;
...
$('#bet_cashout_button').click(function(){
    if(!user.connected){
        window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1280&response_type=token&state=Meh&redirect_uri=https://bustasatoshi.xyz";
        return;
    }
    if(betting){
        cashedOut = true;
...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    $('#bust_box').html("<span>"+parseFloat((g_currentAt-1)<1?0:(g_currentAt-1)).formatMoney(2,'.',',')+"</span>x");
    ...
    getHash(function(){
        ...
        $.ajax({
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                $('#bet_input').attr("disabled", false);
                $('#cashout_input').attr("disabled", false);
                $('#bust_box').html("Crashed @ <span>"+parseFloat((g_currentAt-1)<1?0:(g_currentAt-1)).formatMoney(2,'.',',')+"</span>x");
                $('#bet_cashout_button').text("Bet");
                ...
                cell1.innerHTML = parseFloat((g_currentAt-1)<1?0:(g_currentAt-1)).formatMoney(2,'.',',');
                ...
                if(AutoRunning){
                    placeBet(parseFloat($('#auto_cashout_input').val()), parseFloat($('#auto_bet_input').val())*parseFloat($('#auto_increase_on_loss_input').val()), 1, parseFloat($('#bet_input').val())*parseFloat($('#auto_increase_on_loss_input').val()));
                }
            }else{ // win
                if(g_stopAt > g_currentAt){
                    ...
                    if(!cashedOut && betting){
                        ...
                        placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt+0.01), parseFloat( ( g_stake * ( g_currentAt + 0.01 ) ) ) );
                    }else{
