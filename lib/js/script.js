...
function placeBet(stopAt, bet, currentAt, stake){
    g_stopAt = stopAt; g_bet = bet; g_currentAt = currentAt; g_stake = stake;
    ...
    $('#bet_cashout_button').click(function(){console.log("Cashed out! @ "+currentAt); cashedOut = true; stopAt=0;});
    ...
    getHash(function(){
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                "payouts": [
                    {from: 0, to: rangeWin, value: ((bet*g_currentAt)*100)},
            ...
            if(data.outcome >= rangeWin){ // loss
                console.log("~LOST~");
                console.log("g_stopAt", g_stopAt);
                console.log("g_currentAt", g_currentAt);
                console.log("g_bet", g_bet);
                console.log("g_stake", g_stake);
                console.log("~~~~");
                ...
                if(g_stopAt <= g_currentAt){
                    ...
                    if(!cashedOut)
                        placeBet( parseFloat(g_stopAt), parseFloat(g_stake), parseFloat(g_currentAt+0.01), parseFloat(stake+((bet*(currentAt+0.01))-bet)) );
                    else{
                        $('#bet_cashout_button').text("Bet");
                        var table = document.getElementById("history_log");
